    ...
    while (true) {
        ...
        if (specail_state == '&') {
            ...
            if (pid != 0) { // parents
                ...
                int file_describe = open(output_file_name.c_str(), O_WRONLY | O_TRUNC | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
                if (file_describe < 0) {
                    perror("open failed");
                    exit(1);
                }
                dup2(file_describe, STDOUT_FILENO); // output content to file_describe
                if (dup2(file_describe, STDOUT_FILENO) == -1) {
                    perror("dup2 failed");
                    close(file_describe);
                    exit(1);
                }
